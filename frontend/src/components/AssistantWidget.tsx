...
export default function AssistantWidget() {
  ...
  // --- NUEVA LÓGICA: Acciones rápidas ---
  function dispatchGlobalEvent(eventName: string) {
    // Dispara el evento que AppLayout escuchará
    window.dispatchEvent(new CustomEvent(eventName, { detail: {} }));
  }

  // CAMBIO: Añadido "Agregar Lead"
  const quickActions = [
    {
      label: "Agregar Lead",
    ...
    <div className="fixed bottom-4 right-4 z-50">
      ...
      {open && (
        ...
        >
          ...
          <div className="p-3 border-t border-soft dark:border-gray-700 bg-app dark:bg-gray-900/50">
            ...
            <div className="flex flex-wrap gap-2">
              {quickActions.map((item) => (
                <button
                  ...
                  onClick={item.action}
                  disabled={working}
                  className="text-xs px-3 py-1.5 rounded-full border border-soft dark:border-gray-700 bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 text-base-clr dark:text-gray-200 disabled:opacity-50"
                >
                  {item.label}
                </button>
              ))}
