...
type PreferenciaBusqueda = {
  ...
  localidad?: string;
  barrio?: string;
  presupuesto_min?: string | number;
  presupuesto_max?: string | number;
  moneda?: string;
...
export default function LeadsPage() {
  ...
  const [matchTarget, setMatchTarget] = useState<Contacto | null>(null);
  ...
  async function handleSaveEdit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (nextContactAtStr) {
      ...
      const now = new Date();
      if (selectedDate < now) {
        alert("La fecha de próximo contacto no puede ser una fecha pasada.");
        return;
      }
    }
    ...
    const prefData = {
        tipo_de_propiedad: formData.get('pref_tipo') || "",
        localidad: formData.get('pref_localidad') || "",
        moneda: formData.get('pref_moneda') || "USD",
        presupuesto_max: formData.get('pref_max') ? Number(formData.get('pref_max')) : null,
        ambientes_min: formData.get('pref_amb') ? Number(formData.get('pref_amb')) : null,
    };
    ...
    const payload = {
        ...
        preferencia: prefData, // Enviamos las preferencias al backend
    };
  ...
  async function handleOpenMatches(c: Contacto) {
      setMatchTarget(c);
      setLoadingMatches(true);
      try {
          const res = await api.get(`contactos/${c.id}/matches/`);
          // Adaptamos por si el backend devuelve directamente el array o un objeto
          if (res.data.resultados) {
             setMatches(res.data.resultados);
          } else {
  ...
  return (
    <div className="relative w-full h-full">
                            ...
                            <tr key={c.id} className="hover:bg-surface-2 transition-colors group">
                                ...
                                <td className="px-5 py-4 text-right">
                                    <div className="flex justify-end gap-2">
                                        ...
                                        <button 
                                            className="h-10 px-4 rounded-lg text-sm font-bold transition-all border border-amber-500 text-amber-600 dark:text-amber-400 dark:border-amber-400 hover:bg-amber-500 hover:text-white dark:hover:bg-amber-500 dark:hover:text-white shadow-sm"
                                            onClick={() => handleOpenMatches(c)}
                                            title="Ver Matches"
                                        >
                                            ✨ Match
                                        </button>
            ...
            {!loading && pageRows.map((c) => {
                ...
                return (
                    <div key={c.id} className="p-4 rounded-xl bg-surface border border-soft space-y-3 shadow-sm">
                        ...
                        <div className="flex justify-end gap-2 pt-2 border-t border-soft flex-wrap">
                            <button
      ...
      {matchTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-4xl bg-surface border border-soft rounded-2xl p-6 shadow-2xl text-base-clr flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center mb-4 shrink-0 border-b border-soft pb-4">
                <div>
                    <h3 className="text-2xl font-black">✨ Propiedades Compatibles</h3>
                    <p className="text-sm text-muted-clr">Para: {matchTarget.nombre} {matchTarget.apellido}</p>
                </div>
                <button onClick={() => setMatchTarget(null)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-surface-2 text-muted-clr hover:text-base-clr text-lg font-bold transition-colors">✖</button>
            </div>
            
            <div className="overflow-y-auto flex-1 custom-scrollbar pr-2">
                {loadingMatches ? (
                    <div className="text-center py-10 animate-pulse font-medium text-muted-clr">Buscando propiedades ideales en tu catálogo...</div>
                ) : matches.length === 0 ? (
                    <div className="text-center py-16 border border-dashed border-soft rounded-xl text-muted-clr bg-surface-2/30">
                        <div className="text-4xl mb-3">🏠</div>
                        <p className="font-bold text-lg text-base-clr">No hay matches aún</p>
                        <p className="text-sm mt-1">Verifica que el lead tenga sus preferencias de búsqueda cargadas (editándolo) y que tengas propiedades disponibles que cumplan esos requisitos.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        {matches.map(p => (
                            <div key={p.id} className="border border-soft rounded-xl p-4 bg-surface-2 hover:shadow-md transition-shadow relative overflow-hidden group">
                                <div className="absolute top-0 left-0 w-1 h-full bg-amber-500"></div>
                                <div className="text-[10px] font-bold text-amber-600 uppercase tracking-wider mb-1">{p.tipo_de_propiedad}</div>
                                <h4 className="font-bold text-base-clr text-base leading-tight mb-1 truncate" title={p.titulo}>{p.titulo}</h4>
                                <div className="text-xs text-muted-clr mb-3 truncate">{p.ubicacion}</div>
                                <div className="font-black text-blue-600 dark:text-blue-400 text-lg mb-3">
                                    {p.moneda} {p.precio?.toLocaleString('es-AR')}
                                </div>
                                <div className="flex gap-2">
                                    <span className="bg-surface px-2 py-1 rounded text-[10px] font-medium border border-soft">{p.ambiente} amb.</span>
                                    {p.superficie && <span className="bg-surface px-2 py-1 rounded text-[10px] font-medium border border-soft">{p.superficie} m²</span>}
                                </div>
                            </div>
                        ))}
                    </div>
      ...
      {editTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-2xl bg-surface border border-soft rounded-2xl p-6 shadow-2xl text-base-clr max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4">
                ...
                <h4 className="text-sm font-bold border-b border-soft pb-2 mt-6 mb-3 text-amber-600 dark:text-amber-500"> Preferencias de Búsqueda (Match)</h4>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-surface-2/50 p-4 rounded-xl border border-soft">
                    <div className="space-y-1">
                        <label className="text-xs uppercase text-muted-clr font-semibold tracking-wider">Tipo Propiedad</label>
                        <select name="pref_tipo" defaultValue={editTarget.preferencia?.tipo_de_propiedad || ""} className="rc-input w-full h-10 cursor-pointer">
                    ...
                    <div className="space-y-1">
                        <label className="text-xs uppercase text-muted-clr font-semibold tracking-wider">Localidad</label>
                        <input name="pref_localidad" defaultValue={editTarget.preferencia?.localidad || ""} className="rc-input w-full h-10" placeholder="Ej: Rosario" />
                    </div>
                    <div className="space-y-1">
                        <label className="text-xs uppercase text-muted-clr font-semibold tracking-wider">Ambientes (Mín)</label>
                        <input name="pref_amb" type="number" min="0" defaultValue={editTarget.preferencia?.ambientes_min || ""} className="rc-input w-full h-10" placeholder="Ej: 3" />
                    </div>
                    <div className="space-y-1">
                        <label className="text-xs uppercase text-muted-clr font-semibold tracking-wider">Moneda</label>
                        <select name="pref_moneda" defaultValue={editTarget.preferencia?.moneda || "USD"} className="rc-input w-full h-10 cursor-pointer">
                            <option value="USD">USD</option>
                            <option value="ARS">ARS</option>
                        </select>
                    </div>
                    <div className="space-y-1 sm:col-span-2">
                        <label className="text-xs uppercase text-muted-clr font-semibold tracking-wider">Presupuesto Máximo</label>
                        <input name="pref_max" type="number" min="0" defaultValue={editTarget.preferencia?.presupuesto_max || ""} className="rc-input w-full h-10" placeholder="Ej: 150000" />
                    </div>
                </div>
